...
def test_create_user(client):
    """Crear usuario nuevo — endpoint público."""
...
def test_get_user_by_auth0(client, test_user):
    """Obtener usuario por auth0_id — endpoint público."""
...
def test_list_users_removed(client):
    """GET /users ya no existe — eliminado por seguridad."""
    response = client.get("/users")
    assert response.status_code == 405


def test_update_user(client, test_user):
    """Actualizar datos del usuario — requiere auth."""
...
def test_delete_user(client, test_user):
    """Eliminar usuario — requiere auth."""
